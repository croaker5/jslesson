...
var post_req = https.request(post_options, function (response) {//第一个参数必须是String或object类型，如果是String的话默认
                                                                 //按照url.parse()方法解析,或者必须是一个url，第二个参数是一个回调函数
                                                                // 即对微信服务器发回本服务器的response添加了几个监听器，以对相应的
                                                                 //response内容作出反应
    var responseText = [];
    var size = 0;
...
// post the data
post_req.write(post_str);//把body的内容写到request里
post_req.end();//发送request